...
- **100% French Translation Coverage**: Ships with complete master template (`languages/woo-get-data-for-ai.pot`), French PO translation (`languages/woo-get-data-for-ai-fr_FR.po`), and binary compiled MO file (`languages/woo-get-data-for-ai-fr_FR.mo`) covering 671+ UI, playbook, and API strings.
...
| `GET /system` | GET | Server, WP/WC/PHP/MySQL versions, runtime performance constants (SAVEQUERIES, SCRIPT_DEBUG, revisions cap, memory limit), active plugins list & summary counts (`?plugins=active\|inactive\|all`, default: `active`), HPOS status, Action Scheduler queue & retention policy |
...
| `GET /performance/caching` | GET | Universal caching & optimization diagnostic: Object Cache (Redis/Memcached), Page Cache drop-in (`advanced-cache.php`), Cloudways Breeze, in-depth WP Rocket settings (RUCSS vs CPCSS mode, CSS safelist, Delay JS exclusions & safe mode, lazyload, mobile caching), and wp-config.php runtime performance constants audit (SAVEQUERIES memory leak check, SCRIPT_DEBUG, revisions cap, memory limits) with strict security redaction |
...
| `GET /tracking/audit` | GET | Master 360° audit of Meta Ads Hybrid Tracking (Pixel + CAPI v21.0), Google Ads Server-Side conversions, cookie banner / CMP detection (native woo-gads, Complianz, Axeptio, Cookiebot), Google Consent Mode v2 signals, inter-plugin cookie name coherence check, and Action Scheduler health |
| `GET /tracking/orders` | GET | Recent orders with server-side tracking metadata, CAPI sync status, Google Ads conversion status, consent states, and click identifiers (`gclid`, `wbraid`, `gbraid`, `fbp`, `fbc`) (`?limit=20`, `?status=`, `?tracking_filter=all\|failed\|missing_consent\|has_identifiers\|meta_failed\|gads_failed`) |
| `GET /tracking/logs` | GET | Unified tracking logs from Google Ads database table (`wp_woo_gads_logs`) and Meta CAPI WooCommerce log files (`wfbt-server-side-*.log`) (`?provider=all\|meta\|google_ads`, `?limit=30`) |
...
- **Built-in Procedural Playbooks (7 MECE Strategic Master Pillars)**:
  ...
  2. `agency_performance_audit` (Agency Multi-Template Performance & Core Web Vitals Audit): Strategic caching and optimization audit (runtime performance constants from wp-config.php including SAVEQUERIES memory leak check, SCRIPT_DEBUG, revisions cap, memory limit, Object Cache, Page Cache drop-in, Cloudways Breeze, WP Rocket RUCSS vs CPCSS, Delay JS exclusions & safe mode, lazyload, mobile caching), 5-template archetypes discovery (Home, Shop, Category, Product, Cart), on-demand profiling (SQL duration/queries per plugin with SAVEQUERIES active warning, TTFB, memory), 100% native server-side Core Web Vitals (DOM size, Elementor nodes %, CLS missing dimensions, legacy image formats, Google Fonts display=swap, core bloat scripts, wc-cart-fragments, server compression), multi-layer cache purging, and active plugins database footprint (`/performance/caching`, `/performance/templates-urls`, `/performance/profile`, `/performance/cache/purge`, `/performance/plugins-summary`).
  3. `database_system_hygiene` (System Health, Database Bloat & Background Hygiene Audit): Unified system infrastructure, memory limits, runtime wp-config.php performance constants (SAVEQUERIES, SCRIPT_DEBUG, WP_POST_REVISIONS cap), database size & top heavy tables, autoload memory bloat with orphaned options detection from inactive plugins, WooCommerce order status distribution & stale unpaid orders (> 1y), Action Scheduler queue backlog & retention policy with bloat alerts, overdue WP-Cron jobs, Crash Watch fatal error summary, and global read-only database/options/snippets search (`/system`, `/system/database`, `/woocommerce/summary`, `/action-scheduler`, `/crons`, `/logs/errors-summary`, `/system/search`).
  4. `order_checkout_troubleshoot` (Orders, Payment Gateways, Emails, PMPro, Tracking & Delivery Troubleshooting): Full e-commerce operational troubleshooting combining recent order failures, payment gateway error notes, coupon/fee inspections, targeted payment gateway logs inspection (Stripe, Alma, PayPal) with automatic hash-free filename resolution, active checkout snippets/hooks, transactional SMTP & MailPoet Sending Service (MSS) deliverability diagnostics (provider detection, credentials redaction, PHP mail() spam risk, bounce alerts), WooCommerce transactional emails inspection with Order Status Manager trigger rules and silent status detection, database email logs search (WP Mail Logging, FluentSMTP, Post SMTP, WP Mail SMTP), MailPoet subscriber bounce check, WooCommerce webhook delivery status, server-side tracking failed order correlation (`/tracking/orders?tracking_filter=failed`), Paid Memberships Pro member/level diagnostics, and MasterStudy LMS user course enrollment & expiration root cause analysis (`/woocommerce/orders`, `/woocommerce/order/{id}`, `/woocommerce/payment-logs`, `/logs/view`, `/snippets`, `/system/mail`, `/woocommerce/emails`, `/logs/emails`, `/system/mail/subscriber`, `/woocommerce/webhooks`, `/tracking/orders`, `/pmpro/member/{user_id}`, `/masterstudy/user/{user_id}/courses`).
  5. `ecommerce_bi_analytics` (360° E-Commerce Sales, Traffic, Conversion, Seasonality & Tracking Analytics): Complete commercial, CRO & tracking intelligence: native WooCommerce sales (gross/net, paid orders, AOV, refunds, % growth vs prior period), advertising seasonality & budget pacing (month decades, payday window effect, day-of-month rankings 1-31, day-of-week, 24h dayparting & budget allocation recommendations), top performing products & coupons, stock valuation & dormant inventory, traffic channels, UTM marketing campaigns, device breakdowns, and Server-Side Tracking & GDPR Consent Compliance Audit (`/woocommerce/analytics/sales`, `/woocommerce/analytics/pacing`, `/woocommerce/analytics/top-performers`, `/woocommerce/analytics/stock`, `/analytics/overview`, `/analytics/campaigns`, `/tracking/audit`).
...
- Commands: `pull:all`, `pull:capabilities`, `pull:skill`, `pull:system`, `pull:scheduler`, `pull:theme`, `pull:code`, `pull:checksums`, `pull:elementor`, `pull:snippets`, `pull:flowmattic`, `pull:analytics`, `pull:meta`, `pull:woocommerce`, `pull:content`, `pull:performance`, `pull:tracking`, `pull:logs`.
...
- **Nouvel Endpoint Dédié d'Audit des Logs Passerelles de Paiement (`GET /woocommerce/payment-logs` & alias `/logs/payment-logs`)** :
  - **Diagnostic Ciblé & Instantané des Incidents de Checkout** : Permet aux agents IA et développeurs d'auditer directement les passerelles de paiement (Stripe, Alma, PayPal, Mollie, PayPlug, Monetico, Systempay, Klarna, Scalapay) sans avoir à lister les sources, deviner le nom de fichier rotatif quotidien ou extraire le hash WooCommerce à 32 caractères.
  - **Résolution Automatique Multi-Passerelles & Aliases Intelligents** :
    - Détection et résolution automatique des handles de logs associés : `stripe` (résout `woocommerce-gateway-stripe`, `eh_stripe_pay`, etc.), `alma` (résout `alma`, `php-client`), `paypal` (résout `woocommerce-paypal-payments`, `paypal`, `ppcp-paypal`), etc.
    - Mode global `gateway=all` (défaut) : scanne et agrège automatiquement les logs de toutes les passerelles actives et détectées sur la boutique.
    - Métadonnées complètes `available_gateways` renvoyées dans chaque réponse avec état d'activation, titre, handles associés, dernier fichier de log généré et date.
  - **Traçage Précis par Commande (`order_id`)** :
    - Filtrage par expression régulière stricte avec frontière de mots pour cibler immédiatement les erreurs ou confirmations associées à un numéro de commande spécifique.
    - Recherche rétroactive automatique sur plusieurs jours récents (`days=5` par défaut si `order_id` renseigné) pour retrouver l'historique d'une commande passée sans être bloqué au seul fichier du jour.
  - **Filtrage par Sévérité PSR-3 (`level`)** :
    - Support de `error` (capture `ERROR`, `CRITICAL`, `EMERGENCY`, `ALERT`), `warning`, `info`, `debug`, `all`.
  - **Moteur de Recherche par Streaming Inverse Haute Performance (`fseek`)** :
    - Lecture en mémoire constante par blocs de 32 Ko en streaming inversé depuis la fin du fichier. Zéro chargement de fichiers volumineux en mémoire RAM PHP (sécurisé contre `memory_limit` même sur des logs de 50 Mo+).
  - **Parsing Structuré & Double Format de Restitution** :
    - Retourne à la fois les lignes brutes caviardées (`lines`) et une structure d'objets analysés (`entries`) avec `timestamp`, `level`, `message`, `context` (JSON décodé et caviardé) et `source_file`.
  - **Blindage Sécurité & Caviardage RGPD Renforcé (`Redaction`)** :
    - Caviardage systématique des clés secrètes Stripe/Alma live et test (`sk_live_...`, `sk_test_...`), clés restreintes (`rk_...`), secrets de webhook (`whsec_...`), client secrets PaymentIntent/SetupIntent (`pi_..._secret_...`), numéros de cartes bancaires (PAN), codes CVC/CVV, et e-mails/téléphones clients.
  - **Permissions & Playbooks** :
    - Accessible de manière transparente si le module `woocommerce` OU le module `logs` est activé.
    - Intégré directement dans l'étape 3 du Pilier 4 (`order_checkout_troubleshoot`).
  - **Synchronisation CLI & i18n** :
    - Intégration de l'extraction des logs de paiement dans `pull:woocommerce` de `cli/sync.js`.
    - Couverture linguistique française maintenue à 100% (742 chaînes) via `cli/sync-i18n.php`.
...
- **Nouvel Endpoint de Pacing & Saisonnalité Publicitaire (`Woocommerce_Controller`)** :
  - **Route Dédiée `GET /woocommerce/analytics/pacing` (Permission: `woocommerce`)** :
    - Conçu spécifiquement pour le media buying et le pilotage des budgets Google Ads & Meta Ads sans retraitements externes.
    - Paramètres supportés : `range` (`last_12_months` [défaut], `last_24_months`, `all_time`, `custom`), `start_date`, `end_date`, `status` (normalisation bidirectionnelle avec et sans préfixe `wc-`), `monthly_budget` (simulation budgétaire euro optionnelle).
    - **1. Répartition par Décades (`decades`)** : `early_month` (jours 1 à 10), `mid_month` (jours 11 à 20), `late_month` (jours 21 à 31) avec % commandes, % CA net, CA moyen/jour, commandes moyennes/jour et panier moyen AOV.
    - **2. Impact Fenêtre de Paie (`pay_window`)** : Analyse comparative entre les jours de paie (jours 25 du mois au 5 du mois suivant) vs cœur du mois hors paie (jours 6 au 24), avec calcul du lift de CA journalier et ratio de vélocité.
    - **3. Classement des Jours du Mois (`days_of_month`)** : Tableau exhaustif de 1 à 31 avec commandes, CA net total, jours observés normalisés selon le calendrier réel, CA moyen/j, commandes/j, panier moyen, part de CA et classement de rentabilité (#1 à #31).
    - **4. Performance par Jour de Semaine (`day_of_week`)** : Répartition détaillée du lundi au dimanche, identification du meilleur et du pire jour de conversion, multiplicateurs de pacing et comparaison semaine vs week-end.
    - **5. Profil Horaire 24h & Dayparting (`hourly_profile`)** : Micro-requête indexée groupée par heure (00h à 23h) détectant les pics de conversion et les heures creuses nocturnes avec recommandations d'ajustements de calendrier de diffusion Google Ads (-40% à -60% la nuit, +15% à +25% sur les pics).
    - **6. Recommandations Actionnables & Simulateur (`recommendations`)** : Ventilation budgétaire recommandée en 3 décades (total 100%), multiplicateurs de budget jour pour la fenêtre de paie et les jours de semaine, simulation immédiate en euros selon le budget mensuel, et synthèse stratégique.
  - **Moteur de Requête Haute Performance (< 150ms)** :
    - Exécution directe sur `{$wpdb->prefix}wc_order_stats` avec fallback automatique sur HPOS `{$wpdb->prefix}wc_orders` et tables d'historique `wp_posts` / `wp_postmeta`.
    - Normalisation calendaire déterministe en mémoire (compte exact des occurrences du jour 31, des lundis, etc.) évitant tout biais d'échantillonnage.
- **Intégration Playbooks & Gouvernance 7 Piliers MECE (`Playbooks`)** :
  - Intégration en Étape 7 du Pilier 5 `ecommerce_bi_analytics` ("Advertising Seasonality & Budget Pacing").
  - Ajout des déclencheurs d'intention media buying (`pacing publicitaire`, `saisonnalite ventes`, `budget google ads`, `budget meta ads`, `ad scheduling`, `dayparting`, `fenetre de paie`, `effet paie`, `decades ventes`, `repartition budget pub`).
- **Synchronisation CLI Locale (`cli/sync.js`)** :
  - Extraction automatique de `analytics-pacing.json` et génération du rapport markdown structuré `pacing-report.md`.
- **Internationalisation 100% Français (`languages/`, `cli/translations-fr.php`)** :
  - 31 nouvelles chaînes traduites et compilées, maintenant un taux de couverture français strict de 100% (740/740 chaînes).
...
- **Audit des Constantes de Performance d'Exécution & Détection SAVEQUERIES (`includes/api/class-system-controller.php`, `class-performance-controller.php`, `class-permissions.php`, `class-playbooks.php`, `cli/translations-fr.php`)** :
  - **Détection des Fuites Mémoire SAVEQUERIES en Production** :
    - Détection proactive de la constante `SAVEQUERIES` dans `get_runtime_performance_constants()`.
    - Alerte critique de sévérité maximale si `SAVEQUERIES === true` sur un site en production : alerte que chaque requête SQL exécutée par WordPress est conservée en mémoire dans `$wpdb->queries` avec les traces d'appel de fonctions, provoquant des fuites de mémoire massives (des dizaines à centaines de Mo par requête) et dégradant drastiquement le TTFB.
    - Prescrit la désactivation immédiate dans `wp-config.php` (`define('SAVEQUERIES', false);`).
  - **Audit Complet des Drapeaux de Performance de `wp-config.php`** :
    - `SCRIPT_DEBUG` : Alerte de niveau avertissement si actif en production (force le chargement des fichiers JS/CSS natifs non minifiés, multipliant le poids des assets).
    - `WP_DEBUG_DISPLAY` : Alerte critique si actif en production (risque d'affichage d'erreurs et de chemins PHP sensibles aux visiteurs).
    - `WP_POST_REVISIONS` : Audit du plafonnement des révisions d'articles et de produits. Alerte si illimité (`true` ou non défini), recommandation de définir `WP_POST_REVISIONS` à 5 pour prévenir le bloat exponentiel de la table `wp_posts`.
    - `WP_MEMORY_LIMIT` & `WP_MAX_MEMORY_LIMIT` : Conversion sécurisée en octets (`parse_memory_bytes()`), recommandation d'allouer au minimum 256M pour WooCommerce.
    - `WP_CACHE` & `DISABLE_WP_CRON` : Vérification des drapeaux de drop-in de cache et de planification cron serveur.
  - **Intégration Transversale dans les Contrôleurs REST** :
    - `GET /system` : Nouvelle clé `'runtime_constants'` exposant les constantes auditées, les alertes formatées et le drapeau `has_critical_alerts`.
    - `GET /system/security` : Ajout des alertes et recommandations pour `SAVEQUERIES` et `SCRIPT_DEBUG`.
    - `GET /performance/caching` (et alias `GET /system/caching`) : Exposition de `'runtime_constants'` et injection automatique des alertes de constantes dans le tableau `$recommendations`.
    - `GET /performance/profile` : Ajout des drapeaux `'savequeries_globally_active'` et `'savequeries_warning'` dans le profilage de page à la demande.
  - **Synchronisation des Playbooks MECE (Piliers 2 & 3)** :
    - **Pilier 2 (`agency_performance_audit`)** : Intégration dans l'Étape 1 (*Caching Infrastructure, Runtime Constants & Performance Flags Audit*), signaux clés `runtime_constants.savequeries` et `runtime_constants.has_critical_alerts`, nouveaux déclencheurs (`savequeries`, `savequeries activé`, `constantes wp-config`).
    - **Pilier 3 (`database_system_hygiene`)** : Intégration dans l'Étape 1 (*Server Limits & Environment Diagnostic*), signaux clés `system.runtime_constants.savequeries`, déclencheurs `savequeries bdd`, `constantes performance`.
    - **Directive Agent 7** mise à jour dans `generate_skill_markdown()` avec avertissement explicite sur l'impact mémoire désastreux de `SAVEQUERIES`.
  - **Internationalisation & Loco Translate 100%** :
    - 22 nouvelles chaînes gettext ajoutées dans `cli/translations-fr.php`.
    - Synchronisation complète via `php cli/sync-i18n.php` : 709/709 chaînes traduites (100%), fichiers `.pot`, `.po` et binaire compilé `.mo` synchronisés.
...
- **Module d'Audit Tracking Server-Side (Meta CAPI & Google Ads) & Conformité RGPD / Consent Mode v2 (`Tracking_Controller`, `Permissions`, `Playbooks`, `sync.js`)** :
  - **Nouveau Contrôleur REST Dédié (`includes/api/class-tracking-controller.php`)** en 100% lecture seule sous le namespace `agent-bridge/v1/tracking/` :
    - `GET /tracking/audit` : Audit 360° du tracking serveur, des pixels frontend et du consentement RGPD en 1 seule requête :
      - *Meta Ads Server-Side (Pixel + Conversions API v21.0)* : Détection du plugin `woo-fb-tracking-server-side`, Pixel ID, Access Token caviardé (longueur et aperçu sûr `EAAB...xyz`), état de l'API de conversions, mode débug/test (`test_event_code`), proxy custom domain, hashing SHA256 des données de matching avancé, et statistiques de la file Action Scheduler (`wfbt_send_capi_event` : en attente, échouées, réussies).
      - *Google Ads Server-Side (API Conversions & Enhanced Conversions)* : Détection du plugin `woo-gads-tracking-server-side`, Conversion ID (`AW-...`), Conversion Labels (`purchase`, `add_to_cart`, `begin_checkout`, `lead`, etc.), conversion tracking direct vs webhook / serveur, Enhanced Conversions (SHA256), mode test, et statistiques de la table SQL `{$wpdb->prefix}woo_gads_logs` (succès, erreurs).
      - *Bannière Cookies, CMP & Google Consent Mode v2* : Détection de la bannière active (bannière native Google Ads, Complianz, Axeptio, Cookiebot, CookieYes, iubenda, etc.), état de Google Consent Mode v2 (`ad_storage`, `analytics_storage`, `ad_user_data`, `ad_personalization`), mode de consentement (Basic vs Advanced), politique de stockage (durée de validité du consentement).
      - *Contrôle de Cohérence des Noms de Cookies de Consentement* : Détection automatique des désynchronisations critiques (ex. Meta CAPI configuré pour écouter `concord_consent` alors que la bannière native Google Ads dépose `woo_gads_consent`), avec diagnostic immédiat et alerte rouge de remédiation.
      - *Analyse de Santé Globale (`health_status` & `alerts[]`)* : Calcul d'un score de santé et liste des alertes/avertissements classés par sévérité (`critical`, `warning`, `info`).
    - `GET /tracking/orders` : Inspection de l'attribution et des statuts de transmission server-side sur les commandes récentes (Meta `_wfbt_capi_sent`, Google Ads `_woo_gads_sent`, IDs d'événements `fb_event_id`, clics `fbclid` / `gclid`, anonymisation PII stricte).
    - `GET /tracking/logs` : Accès unifié et streaming mémoire sécurisé aux logs d'événements server-side : logs Meta CAPI depuis WC Logger (`wfbt-*`) et logs Google Ads depuis la table SQL `{$wpdb->prefix}woo_gads_logs` avec filtrage par statut (`all`, `error`, `success`), pagination et masquage des données sensibles.
  - **Gouvernance des Playbooks (Règle MECE des 7 Piliers Majeurs)** :
    - Aucun 8ème playbook n'a été créé (respect de la règle d'or MECE) : intégration fluide du tracking dans le **Pilier 5 : `ecommerce_bi_analytics`** (Étape 6 : « Tracking Server-Side (Meta CAPI & Google Ads) & Consentement RGPD ») et ajout de `'tracking'` dans les modules optionnels des Piliers 4 et 5.
    - Nouveaux déclencheurs d'intention : `tracking server side`, `capi meta`, `google ads conversions`, `pixel facebook`, `consent mode v2`, `bannière cookies`, `rgpd tracking`, `capi event id`, `wfbt`, `woo gads`.
  - **Matrice des Permissions & Documentation d'Administration** :
    - Déclaration du module `'tracking'` (*Tracking Server-Side (Meta, Google) & RGPD*) dans `Permissions::get_module_definitions()` activé par défaut (`1`).
    - Documentation des endpoints dans l'onglet Documentation de l'administration (`tab-docs.php`).
  - **Client CLI Local (`cli/sync.js`)** :
    - Commande `pull:tracking` ajoutée, avec extraction de `tracking-audit.json`, `tracking-orders.json`, `tracking-logs.json` et génération du rapport Markdown exécutif `tracking-report.md`. Intégrée dans `pull:all`.
  - **Internationalisation & Loco Translate 100%** :
    - Synchronisation complète via `cli/sync-i18n.php` avec 671/671 chaînes traduites en français dans `woo-get-data-for-ai-fr_FR.mo`.
...
- **Moteur SEO Unifié Multi-Plugins (Rank Math, Yoast SEO, The SEO Framework) & Inspection Complète des Redirections (`Content_Controller`, `Rest_Controller`, `Performance_Controller`, `Permissions`, `Playbooks`, `sync.js`)** :
  - **Support SEO Unifié Approfondi** :
    - **Rank Math SEO (Gratuit & Pro)** : Détection proactive, lecture des métadonnées `rank_math_*` (title, description, canonical, robots noindex/nofollow/advanced, focus keyword, OpenGraph/Twitter cards, redirection, score SEO).
    - **Yoast SEO (Gratuit & Premium)** : Détection native (`WPSEO_VERSION`, `WPSEO_PREMIUM_FILE`, WooCommerce SEO `WPSEO_WOO_VERSION`), lecture des métadonnées `_yoast_wpseo_*` (title, metadesc, canonical, meta-robots-noindex/nofollow/adv, opengraph/twitter, primary category, focus keyword, scores SEO/readability).
    - **The SEO Framework (TSF / Autodescription)** : Détection native (`THE_SEO_FRAMEWORK_VERSION`), lecture des métadonnées postmeta `_genesis_title`, `_genesis_description`, `_genesis_canonical_uri`, `_genesis_noindex`, `_genesis_nofollow`, `_genesis_noarchive`, `_open_graph_title`, `_open_graph_description`, `_social_image_url`, `_twitter_title`, `_twitter_description`, `_tsf_twitter_card_type`, `redirect`, et term meta `autodescription-term-settings`.
    - **Résolution des Modèles Dynamiques** : Support bidirectionnel des balises de template de Yoast (`%%title%%`, `%%sitename%%`, `%%sep%%`, etc.) et de Rank Math (`%title%`, `%sitename%`, `%sep%`, etc.).
  - **Nouveaux Endpoints d'Inspection des Redirections & Logs 404** :
    - `GET /content/redirections` : Liste, filtre (`status_code`, `search`, `provider`) et pagine les règles de redirection configurées (301, 302, 307, 410) à travers **Redirection** (John Godley, table `wp_redirection_items`), **Rank Math Redirections** (table `wp_rank_math_redirections`), et **301 Redirects** (option `eps_redirects`).
    - `GET /content/redirections/404` : Inspection des logs d'erreurs 404 récents enregistrés par l'extension Redirection (table `wp_redirection_404`) avec comptage des hits par URL, référent et **anonymisation stricte RGPD/PII de l'IP cliente**.
  - **Intégration Pilier 1 MECE (`seo_content_audit`) & Matrice de Permissions** :
    - Mise à jour du module `'content'` (rebaptisé *Pages, Contenus, SEO & Redirections*).
    - Enrichissement du playbook Pilier 1 avec déclencheurs d'intention (migrations SEO, dépannage 404, audit de redirections), signaux d'indexation préventifs et Étape 4 dédiée aux redirections.
  - **Intégration Client CLI (`cli/sync.js`) & i18n 100%** :
    - Commande `pull:content` enrichie avec téléchargement automatique de `redirections.json`, `404-logs.json` et génération du rapport `redirections.md`.
    - Synchronisation i18n avec 100% de couverture française (497/497 chaînes traduites).
