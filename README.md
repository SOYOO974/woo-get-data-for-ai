...
- **Deep WooCommerce Diagnostics**: Audits template overrides in child/parent themes, detects outdated templates, and inspects HPOS status, HPOS datastore caching, product object caching, deferred transactional emails, checkout rate limiting, and full-text search indexes.
...
- **SMTP, MailPoet MSS & Email Delivery Diagnostics**: Provider detection (**FluentSMTP**, **WP Mail SMTP**, **Post SMTP**, **MailPoet / MSS**), credentials sanitization, MailPoet subscriber bounce check (`/system/mail/subscriber`), WooCommerce transactional emails and custom trigger conditions (`/woocommerce/emails`), and database email logs search (`/logs/emails` supporting **WP Mail Logging**, **FluentSMTP**, **Post SMTP**, **WP Mail SMTP**).
- **Security Hardening & Protection Audit**: Audit of constants (`DISALLOW_FILE_EDIT`, `DISALLOW_FILE_MODS`, `WP_DEBUG_DISPLAY`, `SAVEQUERIES`, `SCRIPT_DEBUG`), XML-RPC exposure, SSL enforcement, DB prefix, detected security and caching plugins.
- **Runtime Performance Constants & Memory Leaks Audit**: Surgical inspection of `wp-config.php` constants with critical alerts: detects production memory leaks caused by `SAVEQUERIES` (recording all SQL queries & backtraces in `$wpdb->queries`), `SCRIPT_DEBUG` unminified asset penalty, `WP_POST_REVISIONS` capping recommendations (5 revisions to prevent `wp_posts` bloat), and `WP_MEMORY_LIMIT` (minimum 256M recommended for WooCommerce).
...
- **WordPress Pages, Content, Unified SEO, Redirections & Custom Post Types**: Complete inspection of WordPress pages hierarchy, raw and rendered Gutenberg block trees, detected shortcodes, templates, dynamic Custom Post Types support on `/content/posts` (`post_type=any` or comma-separated list), dedicated registered CPT inventory (`/content/custom-post-types` with publication status counts), and **unified SEO metadata** normalized across **Rank Math**, **Yoast SEO**, **The SEO Framework (TSF)**, **SEOPress**, and **All in One SEO**, alongside global SEO plugin settings & optimization health audits (`/content/seo/settings`), 301/302/410 URL redirect rules, and 404 error log monitoring (**Redirection** by John Godley, **Rank Math Redirections**, **301 Redirects**) with site-wide audit capabilities across pages, blog posts, WooCommerce products, and categories.
- **WooCommerce Account Area & Attached Theme Templates**: Deep inspection of the WooCommerce "My Account" space (`/woocommerce/account-tabs`): navigation menu items, endpoint URLs, action hooks and callback identification via PHP Reflection (without session execution side-effects), extraction of **Woodmart** theme custom tabs (`xts-woodmart-options['my_account_custom_tabs']`), dashboard text/template, and automatic extraction of attached Woodmart HTML blocks (`cms_block`) or Elementor templates with raw content.
- **Global Read-Only System Search**: Fast, memory-safe unified search (`/system/search`) across `$wpdb->posts` (pages, posts, `cms_block`, `elementor_library`, `woodmart_layout`) with contextual ±80 character snippets, `$wpdb->options` (sensitive keys redacted, transients excluded), and WPCode / Code Snippets with direct WordPress admin edit URLs.
...
- **Server-Side Tracking (Meta CAPI & Google Ads) & GDPR Consent Mode v2**: Deep audit of server-side conversion tracking (`/tracking/audit`), inspecting Meta CAPI (`woo-fb-tracking-server-side`, Access Token redaction, Action Scheduler queue `wfbt_send_capi_event`), Google Ads Conversions API & Enhanced Conversions (`woo-gads-tracking-server-side`, Conversion IDs/labels, `wp_woo_gads_logs`), cookie banner & Google Consent Mode v2 status, consent cookie name coherence checks (e.g. `concord_consent` vs `woo_gads_consent`), recent order attribution flags (`/tracking/orders`), and unified CAPI/Google Ads log streaming (`/tracking/logs`).
- **Automatic Updates via GitHub**: Fully integrated with `plugin-update-checker` (PUC v5.6).
- **Auto Update Checker & 1-Click Admin Banner**: Automatically inspects GitHub releases in the background when navigating to `/wp-admin/options-general.php?page=wp-agent-bridge` (with a 60-second anti-flood cooldown). Displays an elegant notification banner with direct 1-click update via native WordPress upgrader, release notes link, header status badge, and manual check button.
...
| `GET /system?plugins={active\|inactive\|all}` | Server limits (PHP, RAM, execution time), WP core info, runtime performance constants (SAVEQUERIES, SCRIPT_DEBUG, revisions cap, memory limit), active plugins list & summary breakdown (total, active, inactive, mu-plugins; default: `active`), HPOS state, Action Scheduler queue, and retention policy. |
...
| `GET /system/mail` | SMTP & transactional email diagnostic: active provider (FluentSMTP, WP Mail SMTP, Post SMTP, MailPoet / MSS), credentials redaction, PHP `mail()` spam risk detection, and recent delivery failures. |
| `GET /system/mail/subscriber?email={email}` | MailPoet subscriber status diagnostic: subscription status (subscribed, unconfirmed, unsubscribed, bounced, inactive), segment memberships, and critical bounce suppression alert. |
| `GET /system/security` | Security hardening audit: `DISALLOW_FILE_EDIT`, `DISALLOW_FILE_MODS`, `WP_DEBUG_DISPLAY`, `SAVEQUERIES`, `SCRIPT_DEBUG`, XML-RPC exposure, SSL enforcement, DB prefix, detected security and caching plugins. |
| `GET /system/search?q={query}&scope={all\|posts\|options\|snippets}&limit={20}` | Global read-only search across `$wpdb->posts` (pages, posts, `cms_block`, `elementor_library`, `woodmart_layout` with ±80 char snippets), `$wpdb->options` (transients excluded, secrets redacted), and WPCode / Code Snippets with direct WordPress admin edit links. |
...
| `GET /logs/errors-summary?limit=15&include_payments=true&days=2` | Crash Watch: aggregated and deduplicated recent fatal PHP errors, exceptions, and payment gateway errors (Stripe, Alma, PayPal) with component attribution. |
| `GET /logs/emails?search={query}&status={all\|sent\|failed}&limit={50}` | Search database email delivery logs across WP Mail Logging (`wp_wpml_mails`), FluentSMTP (`fluentmail_log`), Post SMTP (`postman_logs`), and WP Mail SMTP with PII email redaction. |
| `GET /logs/payment-logs` | Alias to `/woocommerce/payment-logs` accessible via either `logs` or `woocommerce` module permissions. |
...
| `GET /woocommerce/settings` | Store configuration: currency, tax settings, stock management, active payment gateways (secrets redacted), shipping zones/methods with locations, flat_rate table rate rules, Flexible Shipping matrix rules, personal data retention policies (`data_retention`), and advanced performance features (`advanced_features`). |
...
| `GET /woocommerce/analytics/pacing?range={range}` | Advertising seasonality & budget pacing: month decades breakdown (days 1-10, 11-20, 21-31), payday window lift (days 25-5 vs 6-24), day-of-month rankings (1-31), day-of-week performance, 24-hour dayparting profile, and automatic budget pacing recommendations for Google Ads & Meta Ads (`?range=last_12_months\|last_24_months\|all_time\|custom`, `?monthly_budget=`). |
...
| `GET /woocommerce/emails` | Inspect registered WooCommerce transactional emails, enabled state, recipients, subject/heading templates, custom triggers from Order Status Manager, and detect silent statuses. |
| `GET /woocommerce/account-tabs` | WooCommerce "My Account" area inspection: navigation tabs, endpoint slugs, URLs, hook callbacks via Reflection, Woodmart theme custom tabs (`xts-woodmart-options['my_account_custom_tabs']`), and attached templates (`cms_block`, Elementor) with raw content. |
...
| `GET /content/posts?status={status}&post_type={type}` | Paginated blog posts and Custom Post Types list (`?post_type=post\|any\|cpt1,cpt2`) with categories, tags, author, editor type, and quick SEO preview. |
...
| `GET /content/custom-post-types` | Inventory of registered public & accessible custom post types with publication status counts (`publish`, `draft`, `trash`, `private`, `total`), supported features, and associated taxonomies. |
...
| `GET /content/redirections/404` | Unified 404 error monitoring (supporting **Redirection** and **Rank Math 404 Monitor**) with structural pattern clustering (`patterns_summary` / `pattern_clusters`), hit frequency, smart in-memory PHP redirection snippet recommendations (`smart_snippet_recommended`), and GDPR/PII-masked client IPs (`?limit=100`, `?provider=redirection\|rank_math`, `?search=`). |
...
| `GET /performance/caching` | Universal caching & optimization diagnostic: Object Cache (Redis/Memcached), Page Cache drop-in (`advanced-cache.php`), Breeze (Cloudways), in-depth WP Rocket settings (RUCSS vs CPCSS mode, CSS safelist, Delay JS exclusions & safe mode, lazyload, mobile caching), and runtime wp-config.php constants audit (SAVEQUERIES memory leak check, SCRIPT_DEBUG, revisions cap, memory limits) with strict security redaction. |
...
| `POST /performance/cache/purge?scope={all\|cdn\|page\|object}` | Multi-layer defensive cache purge: Rocket.net Edge CDN (Cloudflare Enterprise), WP Rocket (domain, minify, busting, RUCSS), Breeze (Page Cache, Minification, Varnish), Object Cache Pro / Redis, LiteSpeed, and Autoptimize (default: `all`). |
...
| `GET /tracking/audit` | Comprehensive server-side tracking audit: Meta Conversions API (CAPI v21.0), Google Ads server conversions, cookie banner & CMP detection, Google Consent Mode v2 signals, consent cookie coherence verification, and health diagnostic alerts. |
| `GET /tracking/orders?status={status}&limit={20}` | Order-level conversion tracking attribution signals: Meta CAPI status (`_wfbt_capi_sent`), Google Ads status (`_woo_gads_sent`), event IDs (`fb_event_id`), and click identifiers (`fbclid`, `gclid`) with strict PII anonymization. |
| `GET /tracking/logs?provider={all\|meta\|google}&status={all\|error\|success}&limit={50}` | Unified server-side tracking event logs: Meta CAPI WC Logger (`wfbt-*`) and Google Ads database logs (`wp_woo_gads_logs`) with secret redaction and error inspection. |
...
1. **360° SEO, Content Hierarchy, Redirections & Visibility Audit** (`seo_content_audit`): 6-step strategic sequence running `/content/seo-audit` (settings audit & indexation blockers), `/content/pages` (hierarchy & templates), `/content/page/{id}` (deep block & meta inspect), `/content/redirections` (301 rules & migration mapping), `/content/redirections/404` (404 structural pattern clustering & smart in-memory redirection snippets recommendation), and `/content/custom-post-types` (registered CPTs inventory with publication status counts).
2. **Agency Multi-Template Performance & Core Web Vitals Audit** (`agency_performance_audit`): Audits caching infrastructure & runtime constants with `/performance/caching` (SAVEQUERIES memory leak check, SCRIPT_DEBUG, revisions cap, memory limit, Object Cache, Page Cache drop-in, Breeze, WP Rocket RUCSS vs CPCSS, Delay JS, safelist), correlates `/performance/templates-urls`, conducts on-demand profiling with `/performance/profile` across 5 archetypes (attributing SQL duration and memory per plugin with SAVEQUERIES active warnings and 100% native Core Web Vitals & frontend signals), invalidates caches with `/performance/cache/purge`, and analyzes active plugins database footprint with `/performance/plugins-summary`.
3. **System Health, Database Bloat & Background Hygiene Audit** (`database_system_hygiene`): Correlates `/system` (including runtime performance constants SAVEQUERIES, revisions cap, memory limits), `/system/database` (table sizes, autoload memory, orphaned options detection), `/woocommerce/summary` (order cancellation ratio & stale unpaid orders > 1y), `/action-scheduler` (queue backlog & retention policy alerts), `/crons` (overdue jobs), `/logs/errors-summary` (Crash Watch), and `/system/search` (global read-only search across posts, options, and code snippets).
4. **Orders, Payment Gateways, Emails, PMPro & LMS Troubleshooting** (`order_checkout_troubleshoot`): Comprehensive checkout and transactional email diagnostics: investigates order failures with `/woocommerce/orders`, `/woocommerce/order/{id}`, gateway debug logs `/logs/view`, active checkout snippets `/snippets`, SMTP & MailPoet Sending Service (MSS) deliverability diagnostics `/system/mail`, WooCommerce transactional emails and custom trigger conditions `/woocommerce/emails`, database email delivery logs `/logs/emails` (WP Mail Logging, FluentSMTP, Post SMTP, WP Mail SMTP), MailPoet subscriber bounce check `/system/mail/subscriber`, WooCommerce webhooks `/woocommerce/webhooks`, Paid Memberships Pro member/level diagnostics `/pmpro/member/{user_id}`, and MasterStudy LMS user course enrollment & expiration root cause analysis `/masterstudy/user/{user_id}/courses`.
5. **360° E-Commerce Sales, Traffic, Conversion & Tracking Analytics** (`ecommerce_bi_analytics`): Deep commercial intelligence combining native WooCommerce sales `/woocommerce/analytics/sales`, `/woocommerce/analytics/top-performers`, `/woocommerce/analytics/stock`, traffic acquisition `/analytics/overview`, marketing campaigns `/analytics/campaigns`, and server-side tracking audit with GDPR Consent Mode v2 `/tracking/audit`, `/tracking/orders`, `/tracking/logs`.
...
7. **Code Architecture, Theme Settings & Automations Map** (`code_theme_integrations`): Audits WooCommerce template version drift `/theme/overrides`, child theme files `/theme/child`, aggregated custom CSS (Customizer, Woodmart responsive blocks) `/theme/custom-css`, cryptographic directory checksum fingerprints for local vs remote drift `/code/checksums`, FlowMattic workflows `/flowmattic/workflows`, Elementor forms `/elementor/forms`, custom snippets (WPCode & Code Snippets) `/snippets`, custom ACF/code meta fields `/meta/fields`, WooCommerce "My Account" area navigation tabs & attached templates (Woodmart `cms_block`, Elementor) `/woocommerce/account-tabs`, and global system search `/system/search`.
...
node sync.js pull:tracking     # Dumps server-side tracking audit, order signals, and logs to ./tracking/
...
- **100% French Translations**: Full master template (`woo-get-data-for-ai.pot`), French PO translation (`woo-get-data-for-ai-fr_FR.po`), and binary compiled MO file (`woo-get-data-for-ai-fr_FR.mo`) included in `woo-get-data-for-ai/languages/` covering 709+ UI, playbook, and API strings.
