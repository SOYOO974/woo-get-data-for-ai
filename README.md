...
1. **360° SEO, Content Hierarchy, Redirections & Visibility Audit** (`seo_content_audit`): 5-step strategic sequence running `/content/seo-audit` (settings audit & indexation blockers), `/content/pages` (hierarchy & templates), `/content/page/{id}` (deep block & meta inspect), `/content/redirections` (301 rules & migration mapping), and `/content/redirections/404` (404 structural pattern clustering & smart in-memory redirection snippets recommendation).
